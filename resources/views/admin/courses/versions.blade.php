                                                    ...
                                                    @foreach($courses as $course)
                                                        <tr>
                                                            <td style="padding: 7px !important;">
                                                                <a href="{{ route('admin.courses.show.version', ['id' => $course->id]) }}" type="button" data-bs-toggle="tooltip" title="Show" class="btn btn-link btn-primary" data-original-title="Show">
                                                                    <i class="fa fa-eye"></i>
                                                                </a>
                                                            </td>
                                                            <td style="padding: 7px !important;">{{ $course->id }}</td>
                                                            @if($course->status === 'draft')
                                                                <td style="padding: 7px !important;" class="text-muted">{{ ucfirst($course->status) }}</td>
                                                            @elseif($course->status === 'pending')
                                                                <td style="padding: 7px !important;" class="text-warning">{{ ucfirst($course->status) }}</td>
                                                            @elseif($course->status === 'rejected')
                                                                <td style="padding: 7px !important;" class="text-danger">{{ ucfirst($course->status) }}</td>
                                                            @elseif($course->status === 'accepted')
                                                                <td style="padding: 7px !important;" class="text-success">{{ ucfirst($course->status) }}</td>
