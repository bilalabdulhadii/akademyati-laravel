...
@section('content')
    ...
    <style>
        ...
        <div class="page-inner" style="min-height: 87vh; margin-top: 100px">
            <div class="container">
                <div class="d-flex justify-content-end gap-2 mb-5">
                    @if($course->status === 'pending')
                        <a class="btn btn-outline-success" data-bs-toggle="modal" data-bs-target="#acceptModal"><i class="fa fa-check me-2"></i>Accept</a>
                        <a class="btn btn-outline-danger" data-bs-toggle="modal" data-bs-target="#rejectModal"><i class="fa fa-times me-2"></i>Reject</a>
                        <!-- Accept Modal Start -->
                        <div class="modal fade" id="acceptModal" data-bs-keyboard="false" tabindex="-1" aria-labelledby="acceptLabel" aria-hidden="true">
                            <div class="modal-dialog modal-dialog-scrollable">
                                <div class="modal-content">
                                    <div class="d-flex justify-content-between p-3">
                                        <h1 class="modal-title fs-5" id="acceptLabel">Accept this version ?</h1>
                                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                    </div>
                                    <div class="modal-body">
                                        <div class="px-4">
                                            <form action="{{ route('admin.courses.version.accept') }}" method="POST" id="infoForm" enctype="multipart/form-data">
                                                @csrf
                                                <input type="hidden" name="course_id" value="{{ $course->id }}">
                                                <p class="mb-2">
                                                    Are you sure you want to accept this course version ? <br>Confirming this
                                                    action will mark the course as approved and it will be published.
                                                    Please verify that all aspects of the course meet the required standards
                                                    before proceeding.
                                                </p>
                                                <button type="submit" class="btn btn-success my-3">Accept</button>
                                            </form>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <!-- Accept Modal End -->
                        <!-- Reject Modal Start -->
                        <div class="modal fade" id="rejectModal" data-bs-keyboard="false" tabindex="-1" aria-labelledby="rejectLabel" aria-hidden="true">
                            <div class="modal-dialog modal-dialog-scrollable">
                                <div class="modal-content">
                                    <div class="d-flex justify-content-between p-3">
                                        <h1 class="modal-title fs-5" id="rejectLabel">Reject this version ?</h1>
                                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                    </div>
                                    <div class="modal-body">
                                        <div class="px-4">
                                            <h3>Rejection Instructions:</h3>
                                            <ol>
                                                <li>State the Reason: Briefly explain why the version is being rejected (e.g., missing details, quality issues).</li>
                                                <li>Provide Feedback: Offer specific suggestions for improvement.</li>
                                            </ol>
                                            <h3>Example Feedback:</h3>
                                            <p>This version was rejected due to missing key topics. Please revise and resubmit.</p>

                                            <form CLASS="mt-5" action="{{ route('admin.courses.version.reject') }}" method="POST" enctype="multipart/form-data">
                                                @csrf
                                                <input type="hidden" name="course_id" value="{{ $course->id }}">
                                                <label class="form-label">Explain why the version was rejected.
                                                    Include specific issues and suggestions for improvement.</label>
                                                <textarea class="form-control editor" id="feedback" name="feedback"
                                                          rows="8" placeholder="Insert a feedback" required></textarea>
                                                <button type="submit" class="btn btn-danger my-3">Reject</button>
                                            </form>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="card">
                            <div class="card-header">
                                <div class="d-flex justify-content-between">
                                    ...
                                    <a href="{{ route('admin.courses.version.redirect', ['id' => $course->id]) }}" target="_blank" class="btn btn-primary rounded-2">Preview</a>
                                </div>
                            </div>
                            <div class="card-body">
                                ...
                                <div class="tab-content mt-3 course-content-tabs mb-3" id="line-tabContent">
                                    ...
                                    <div class="tab-pane fade" id="pricing" role="tabpanel" aria-labelledby="pricing-tab">
                                        <div class="form-group pt-4">
                                            ...
                                            <input type="number" class="form-control" id="course_price" value="{{ number_format($course->price, 2) ?? '' }}" disabled>
                                        </div>
                                    </div>
